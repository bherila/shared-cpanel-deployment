...
check "webroot, cron and stable hooks complete before serving" test "$activate" -lt "$webroot" -a "$webroot" -lt "$cron" -a "$cron" -lt "$post_activate" -a "$post_activate" -lt "$serve"
