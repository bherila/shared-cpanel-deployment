...
existing_audit=$(line_of 'name: Audit existing operational state before quiescence')
...
check "operational candidate audit gates selection after candidate hooks" test "$pre_activate" -lt "$candidate_audit" -a "$candidate_audit" -lt "$activate"
