...
#   atomic-release.sh activate <app> <release>
#   atomic-release.sh serve <app> <release> <php>
#   atomic-release.sh commit <app> <release>
...
artisan_mode() {
    ...
    (cd "$root" && "$php" artisan "$mode" --no-ansi)
}
...
require_maintenance() {
    local php=$1 root=$2 status=0
    maintenance_state "$php" "$root" || status=$?
...
report_status() {
    local php=${1:-} target live_root release commit state maintenance_status=0
    ...
    case $target in
        ...
        legacy)
            ...
            if [ -n "$php" ]; then
                maintenance_state "$php" "$stable" || maintenance_status=$?
        ...
        *)
            if validate_release_target "$target"; then
                ...
                if [ -n "$php" ]; then
                    maintenance_state "$php" "$live_root" || maintenance_status=$?
                    case $maintenance_status in 0) state=maintenance ;; 1) state=serving ;; *) state=unavailable ;; esac
...
begin() {
    ...
    local commit=$1 lock_seconds=$2 retain=$3 failure_policy=$4 initial_live_commit=$5
    shift 5
...
quiesce() {
    [ "$#" -eq 1 ] || { echo "usage: ... quiesce <app> <release> <php>" >&2; exit 2; }
    local php=$1 previous root state=0
    require_owner
    ...
    if [ "$previous" != none ]; then
        ...
        maintenance_state "$php" "$root" || state=$?
        case $state in
    ...
    if [ "$previous" != none ]; then
        artisan_mode "$php" "$root" down
        require_maintenance "$php" "$root"
    fi
...
prepare() {
    [ "$#" -eq 1 ] || { echo "usage: ... prepare <app> <release> <php>" >&2; exit 2; }
    local php=$1 previous legacy_id legacy_target legacy_root path candidate_path converted=false status source_real destination_real
    require_owner
    ...
    if [ "$previous" = legacy ]; then
        ...
        printf 'release=%s\ncommit=%s\ncreated_at=%s\n' "$legacy_id" "$REPLY" "$(date -u +%FT%TZ)" >"$legacy_root/.deploy-release"
...
serve() {
    [ "$#" -eq 1 ] || { echo "usage: ... serve <app> <release> <php>" >&2; exit 2; }
    local php=$1 target
    require_owner
    ...
    artisan_mode "$php" "$candidate" up
    maintenance_state "$php" "$candidate" || maintenance_status=$?
    case $maintenance_status in
        1) ;;
        0) echo "::error::Laravel remains in maintenance after artisan up." >&2; return 1 ;;
        *) echo "::error::Laravel serving state could not be verified after artisan up." >&2; return 1 ;;
    esac
...
commit_release() {
    [ "$#" -eq 0 ] || { echo "usage: ... commit <app> <release>" >&2; exit 2; }
    require_owner
...
finalize() {
    [ "$#" -eq 1 ] || { echo "usage: ... finalize <app> <release> <php>" >&2; exit 2; }
    local php=$1 committed=false risk_started=false recovery_required=false policy=maintenance previous=none current recovery_status=0 conversion_target=
    ...
    if [ "$committed" = true ]; then
        ...
        if [ "$policy" = rollback ] && [ "$previous" != none ]; then
            ...
            if [ "$current" = ".deployments/$app_name/releases/$release_id" ]; then
                artisan_mode "$php" "$candidate" down || recovery_status=1
                if [ "$recovery_status" -eq 0 ]; then switch_stable "$previous" || recovery_status=1; fi
            fi
            current=$(selected_target)
            if [ "$recovery_status" -eq 0 ] && [ "$current" = "$previous" ]; then
                restore_cron || recovery_status=1
                if [ "$recovery_status" -eq 0 ]; then artisan_mode "$php" "$HOME/$previous" up || recovery_status=1; fi
            else
            ...
            if [ "$current" != none ] && [ "$current" != invalid ]; then
                if [ "$current" = legacy ]; then
                    artisan_mode "$php" "$stable" down || recovery_status=1
                else
                    artisan_mode "$php" "$HOME/$current" down || recovery_status=1
                fi
            fi
        ...
        if [ "$previous" != none ]; then
            ...
            if [ "$recovery_status" -eq 0 ]; then
                if [ "$current" = legacy ]; then artisan_mode "$php" "$stable" up || recovery_status=1
                else artisan_mode "$php" "$HOME/$current" up || recovery_status=1; fi
            fi
        fi
        restore_cron || recovery_status=1
    ...
    report_status "$php" || recovery_status=1
    if [ "$recovery_status" -eq 0 ]; then
...
case $command in
    ...
    finalize) finalize "$@" ;;
    status) [ "$#" -le 1 ] || exit 2; report_status "${1:-}" ;;
